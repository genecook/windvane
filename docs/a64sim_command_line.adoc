...
link:a64sim_license.html[License, Copyright Notice]
...
A64simulator/MP - Tuleta Software Co. Copyright 2019. All rights reserved.
...
A64simulator/MP - Tuleta Software Co. Copyright 2018. All rights reserved.
...
Core count
----------
The '--num_cores' command line argument may be used to specify how many processor cores are to be simulated. The default number of cores is one. The maximum number of cores for this release is limited to 256 (a single cluster with a maximum # of cores of 256. Probably will rethink this as its probably a more reasonable implementation
to have multiple clusters of say four cores each).
...
Use the '--seed' command line argument to specify the initial random seed value to use when the _Freerun_ mode is enabled.
...
Show disassembly
----------------
The '--show_disassembly' command option, when present, causes the disassembly for each instruction simulated to be printed to stdout during simulation.
At this time the assembly language statements produced are not guaranteed to assembly correctly.

_Semihosting_ command line options
----------------------------------
For convenience, and of course to make simulation interesting, support for the _ARM_ semihosting functionality has been included.

Via the _semihosting_ interface, calls are provided to
read/write characters and strings to stdout and stdin, or from a file. Calls are also provided to get access to command line parameters. There are also
calls to obtain the time-of-day and other timer related values. The a64sim simulator supported semihosting calls are documented elsewhere. The a64sim
command line options related to semihosting are described below...

The '--app_cmdline' command line option may be used to specify a command line string to pass along onto a simulated program via the semihosting interface. Put another
way, when a simulated program calls the semihosting interface to obtain the command line, the '--app_cmdline' argument value is returned. Quote the string of
course as required to protect embedded blanks or control characters.

The '--app_env' command line option may be used to specify a string containing environment variable definitions, to pass along onto a simulated program via the semihosting interface.

The '--heap_base', '--heap_limit', '--stack_base', '--stack_limit' command line arguments may be used to set stack and heap parameters, again to be passed along
to a simulated program via the semihosting interface. There are no default values for these parameters.

The '--env_ptr', '--cmdline_ptr' command line arguments may be used to specify where in main memory the buffers containing the environment and command-line are
located. There are no default values for these parameters.
