...
-----------------------
The *a64sim* simulator is configured at present with a single cluster of
up to 256 cores, and a single bank of memory shared by all cores.

All cores have the same configation...

.*a64sim* processor configuration
----
; reset state...
[PSTATE]
  N=0              ;
...
[ID_AA64MMFR0_EL1]
  TGran4=1         ; 4k mem blocks supported
  TGran64=1        ; 64k mem blocks supported
  TGran16=1        ; 16k mem blocks supported
  ...
  PARange=0        ; physical address range is 32 bits
[ID_AA64PFR0_EL1]
...
[CPTR_EL2]
  TCTAP=0          ; accesses to CPACR do not trap
